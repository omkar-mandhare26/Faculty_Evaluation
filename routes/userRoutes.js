...
router.get("/get-session-conducted-records", userAuthenticateToken, async (req, res) => {
    const token = req.cookies.token || req.header("Authorization")?.replace("Bearer ", "");
...
router.post("/update-session-conducted-records", userAuthenticateToken, async (req, res) => {
    ...
    try {
        ...
        for (let subjectKey of subjects) {
            ...
            // Check if month is a number and convert it to a string format for better consistency
            const numericMonth = typeof month === "number" ? String(month).padStart(2, "0") : month;

            // Find the existing record using user, subject, month, and year
            let sessionConducted = await noOfLectures.findOne({
                user: user._id,
                subject,
                month: getMonthName(numericMonth).month, // Ensure you're querying by the correct month format
                year
            });
            ...
            if (!sessionConducted) {
                sessionConducted = new noOfLectures({
                    ...
                    sessionAchievement,
                    weightageERP,
                    month: getMonthName(numericMonth).month, // Save the month as a number (string padded to two digits)
                    year,
                    marksAchieved,
                    evaluation,
                    remark
                ...
                // If record exists, update the fields
                ...
                sessionConducted.month = getMonthName(numericMonth).month; // Update month correctly
