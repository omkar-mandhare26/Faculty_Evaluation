document.addEventListener("DOMContentLoaded", () => {
    ...
    searchBtn.addEventListener("click", async () => {
        ...
        try {
            const response = await fetch(`/users/search-session-records?month=${month}&year=${year}`);
            const data = await response.json();

            if (data && !data.isError) {
                renderSubjects(data.records); // Populate records on successful response
            } else {
...
const apiUrl = "/users/get-session-conducted-records";
